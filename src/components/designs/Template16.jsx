...
const Template16 = ({}) => {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-start gap-4">
      ...
      <div className="w-full lg:w-[1200px] grid grid-cols-1 lg:grid-cols-12 px-6 lg:px-32">
        ...
        <div className="col-span-12 px-4 py-6">
          ...
          {/* <div
            ...
            ref={resumeRef}
          > */}
           <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-300" id="resume"
            ref={resumeRef}>
      {/* Header Section */}
      <div className="header-section p-5 rounded-lg border-b border-gray-300">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">{formData.name}</h1>
        <p className="text-lg text-blue-600">{formData.professionalTitle}</p>
      </div>

      {/* About Me Section */}
      <div className="about-section mt-6 p-4 bg-gray-100 rounded-lg border border-gray-300">
        <p className="uppercase text-lg font-bold text-gray-700 mb-2">About Me</p>
        <textarea
          readOnly={!isEdit}
          className={`w-full p-3 rounded-lg text-gray-700 bg-white focus:bg-blue-50 border border-gray-300 ${isEdit ? "focus:border-blue-500" : ""}`}
      ...
      <div className="experience-section mt-6 p-4 bg-gray-100 rounded-lg border border-gray-300">
        <p className="uppercase text-lg font-bold text-gray-700 mb-4">Work Experience</p>
        <div className="experience-list">
              ...
              >
                <div className="flex gap-4">
                  ...
                  <textarea
                    ...
                    name="year"
                    readOnly={!isEdit}
                    className={`w-1/4 p-3 bg-white border border-gray-300 rounded-lg text-gray-700 focus:bg-blue-50 ${isEdit ? "focus:border-blue-500" : ""}`}
                    style={{ resize: "none" }}
                  />
                  <div className="flex-1">
                    ...
                    <textarea
                      ...
                      name="title"
                      readOnly={!isEdit}
                      className={`w-full p-3 bg-white border border-gray-300 rounded-lg text-gray-700 focus:bg-yellow-50 ${isEdit ? "focus:border-yellow-500" : ""}`}
                      style={{ resize: "none" }}
                    />
                    {/* Company and Location Field */}
                    <textarea
                      value={exp.companyAndLocation}
                      onChange={(e) => handleExpChange(i, e)}
                      name="companyAndLocation"
                      readOnly={!isEdit}
                      className={`w-full p-3 bg-white border border-gray-300 rounded-lg text-gray-700 focus:bg-green-50 ${isEdit ? "focus:border-green-500" : ""}`}
                      style={{ resize: "none" }}
                    />
                    {/* Description Field */}
                    <textarea
                      ...
                      name="description"
                      readOnly={!isEdit}
                      className={`w-full p-3 bg-white border border-gray-300 rounded-lg text-gray-700 focus:bg-red-50 ${isEdit ? "focus:border-red-500" : ""}`}
                      style={{ resize: "none" }}
                    />
                  </div>
                  {/* Delete Icon */}
                  {isEdit && (
                    <FaTrash className="text-red-600 cursor-pointer mt-3" onClick={() => removeExperience(i)} />
                  )}
                </div>
              </motion.div>
            ))}
          ...
          {isEdit && (
            <div className="flex justify-end mt-4">
              <FaPlus className="text-blue-600 cursor-pointer" onClick={addExperience} />
            </div>
          )}
        </div>
      </div>
      ...
      {/* Skills Section */}
      <div className="skills-section mt-6 p-4 bg-gray-100 rounded-lg border border-gray-300">
        <p className="uppercase text-lg font-bold text-gray-700 mb-4">Skills</p>
        <div className="skills-list grid grid-cols-2 gap-4">
          <AnimatePresence>
            {skills.map((skill, i) => (
              <motion.div key={i} className="skill-item flex gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {/* Skill Name */}
                <textarea
                  value={skill.title}
                  ...
                  name="title"
                  readOnly={!isEdit}
                  className={`w-full p-3 bg-white border border-gray-300 rounded-lg text-gray-700 focus:bg-purple-50 ${isEdit ? "focus:border-purple-500" : ""}`}
                  style={{ resize: "none" }}
                />
                {/* Skill Percentage */}
                <textarea
                  ...
                  name="percentage"
                  readOnly={!isEdit}
                  className={`w-1/4 p-3 bg-white border border-gray-300 rounded-lg text-gray-700 focus:bg-teal-50 ${isEdit ? "focus:border-teal-500" : ""}`}
                  style={{ resize: "none" }}
                />
                {/* Delete Skill Icon */}
                {isEdit && (
                  <FaTrash className="text-red-600 cursor-pointer mt-2" onClick={() => removeSkill(i)} />
                )}
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          {isEdit && (
            <div className="flex justify-center mt-4">
              <FaPlus className="text-blue-600 cursor-pointer" onClick={addSkill} />
            </div>
          )}
        </div>
      </div>
