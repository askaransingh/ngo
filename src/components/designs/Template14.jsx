      ...
      <div className="w-full lg:w-[1200px] grid grid-cols-1 lg:grid-cols-12 px-6 lg:px-32">
        ...
        <div className="col-span-12 px-4 py-6">
          ...
          {/* <div
            ...
            ref={resumeRef}
          > */}
            <div className="bg-gray-100 p-6 rounded-lg shadow-lg" id="resume"
            ref={resumeRef}>
      <div className="bg-white p-5 rounded-lg shadow-md border border-gray-300">
        <div className="flex flex-col items-start">
          <h1 className="text-4xl font-bold text-indigo-600">{formData.name}</h1>
          <p className="text-xl text-gray-700 font-medium">{formData.professionalTitle}</p>

          {/* About Me */}
          <div className="w-full mt-6">
            <p className="uppercase text-xl font-semibold text-indigo-600">About Me</p>
            <textarea
              readOnly={!isEdit}
              className={`w-full p-3 border rounded-lg text-gray-700 bg-indigo-100 ${isEdit ? "border-indigo-500" : "bg-white"}`}
          ...
          {/* Experience */}
          <div className="w-full mt-6">
            <p className="uppercase text-xl font-semibold text-indigo-600">Work Experience</p>
            <div className="mt-4">
              <AnimatePresence>
                {experiences.map((exp, i) => (
                  ...
                  >
                    <textarea
                      ...
                      name="year"
                      readOnly={!isEdit}
                      className={`w-1/4 p-3 border rounded-lg text-gray-700 bg-yellow-100 ${isEdit ? "border-yellow-500" : "bg-white"}`}
                      style={{ resize: "none" }}
                    />
                    <div className="flex-1">
                      <textarea
                        ...
                        name="title"
                        readOnly={!isEdit}
                        className={`w-full p-3 border rounded-lg text-gray-700 bg-green-100 ${isEdit ? "border-green-500" : "bg-white"}`}
                        style={{ resize: "none" }}
                      />
                      <textarea
                        ...
                        readOnly={!isEdit}
                        className={`w-full p-3 border rounded-lg text-gray-700 bg-blue-100 ${isEdit ? "border-blue-500" : "bg-white"}`}
                        style={{ resize: "none" }}
                      />
                      <textarea
                        ...
                        name="description"
                        readOnly={!isEdit}
                        className={`w-full p-3 border rounded-lg text-gray-700 bg-red-100 ${isEdit ? "border-red-500" : "bg-white"}`}
                        style={{ resize: "none" }}
                      />
                    </div>
                    {isEdit && (
                      <FaTrash className="text-red-600 cursor-pointer" onClick={() => removeExperience(i)} />
                    )}
                  </motion.div>
                ))}
              </AnimatePresence>
              {isEdit && (
                <div className="flex justify-end mt-2">
                  <FaPlus className="text-indigo-600 cursor-pointer" onClick={addExperience} />
                </div>
              )}
            </div>
          </div>
          ...
          {/* Skills */}
          <div className="w-full mt-6">
            <p className="uppercase text-xl font-semibold text-indigo-600">Skills</p>
            <div className="mt-4 flex flex-wrap gap-4">
              <AnimatePresence>
                {skills.map((skill, i) => (
                  <motion.div key={i} className="flex items-center w-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <textarea
                      value={skill.title}
                      onChange={(e) => handleSkillsChange(i, e)}
                      name="title"
                      readOnly={!isEdit}
                      className={`flex-1 p-3 border rounded-lg text-gray-700 bg-purple-100 ${isEdit ? "border-purple-500" : "bg-white"}`}
                      style={{ resize: "none" }}
                    />
                    <textarea
                      ...
                      name="percentage"
                      readOnly={!isEdit}
                      className={`w-1/4 p-3 border rounded-lg text-gray-700 bg-orange-100 ${isEdit ? "border-orange-500" : "bg-white"}`}
                      style={{ resize: "none" }}
                    />
                    {isEdit && (
                      <FaTrash className="text-red-600 cursor-pointer" onClick={() => removeSkill(i)} />
                    )}
                  </motion.div>
                ))}
              </AnimatePresence>
              {isEdit && (
                <div className="w-full flex justify-center mt-2">
                  <FaPlus className="text-indigo-600 cursor-pointer" onClick={addSkill} />
                </div>
              )}
            </div>
          </div>
